...
class SaleBasketLineComponentTest extends CBitrixComponent
{
	...
	{
		$propCode = 'ARTNUMBER';//в данном случаи артикул
		$arProductData = getProductProps(array_merge($arElementId, $arSku2Parent), ['ID', 'PROPERTY_'.$propCode]);

		//условный набор расширений для поиска картинок
		$arrExt = ['jpg', 'jpeg', 'png'];
		...
		foreach ($arBasketItems as &$arItem)
		{
			...
			if (array_key_exists($arItem["PRODUCT_ID"], $arProductData) && !empty($arProductData[$arItem["PRODUCT_ID"]]['PROPERTY_'.$propCode.'_VALUE'])) {
				$artNumber = $arProductData[$arItem["PRODUCT_ID"]]['PROPERTY_'.$propCode.'_VALUE'];

				foreach ($arrExt as $ext) {
					$filename = '/upload/photo/'.$artNumber.'.'.$ext;
					if (@file_exists($_SERVER['DOCUMENT_ROOT'].$filename)) {
						$arItem["PICTURE_SRC"] = $filename;
						break;
					}
				}
			}
